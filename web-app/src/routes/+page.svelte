<script lang="ts">
    ...
    let currentDirection = $state("start");

    async function handleStartClick() {
        currentDirection = "stop";
        await fetch("http://192.168.4.1/move?direction=start", {
            method: "GET",
            headers: {
...
<div class="min-h-screen flex items-center justify-center p-8">
    <div class="flex flex-col items-center justify-center gap-16 bg-zinc-900 border-4 rounded-xl drop-shadow-[8px_8px_0_black] p-8">
        ...
        {#if currentDirection === "start"}
            <Button onclick={handleStartClick} class="h-20 w-40 text-2xl font-semibold border-4 rounded-xl drop-shadow-[8px_8px_0_black] mb-8" variant="outline">
                START
            </Button>
        {:else}
